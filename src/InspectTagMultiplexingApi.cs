...
namespace InspectTagMultiplexingApi
{
    class Program
    {
        ...
        static void Run()
        {
            ...
            string outFile = @"C:\Users\Admin\Documents\Automation\valveDemo2\tag_multiplex_api.txt";
            using (var w = new StreamWriter(outFile))
            {
                w.WriteLine("=== HmiTagType and Multiplexing Inspection ===");
                Assembly asm = typeof(HmiTag).Assembly;
                foreach (Type t in asm.GetTypes())
                {
                    if (t.Name.Contains("Multiplex") || t.Name.Contains("IndexTag"))
                    {
                        w.WriteLine("Type: " + t.FullName);
                        foreach (var m in t.GetMembers()) w.WriteLine("  Member: " + m.Name);
                    }
                }
            }
