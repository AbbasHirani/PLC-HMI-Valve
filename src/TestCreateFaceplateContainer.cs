...
namespace TestCreateFaceplateContainer
{
    class Program
    {
        ...
        static void Run()
        {
            ...
            string outFile = @"C:\Users\Admin\Documents\Automation\valveDemo2\fpc_creation_test.txt";
            using (var w = new StreamWriter(outFile))
            {
                w.WriteLine("=== ScreenItems.Create methods ===");
                var createMethods = sc.ScreenItems.GetType().GetMethods();
                foreach (var m in createMethods)
                {
                    w.WriteLine("Method: " + m.Name);
                    foreach (var p in m.GetParameters())
                    {
                        w.WriteLine("  Param: " + p.Name + " (" + p.ParameterType.FullName + ")");
                    }
                }
            }
