...
class Program {
  ...
  static void Main(){
    ...
    foreach(var nm in new[]{"HmiWindowFlag","HmiStatusBarPart","HmiToolBarPart"}){
      var t=all.FirstOrDefault(x=>{try{return x.Name==nm;}catch{return false;}});
      Console.WriteLine("\n=== "+nm+" ===");
      if(t==null){Console.WriteLine("  not found");continue;}
      if(t.IsEnum){ foreach(var v in Enum.GetValues(t)) Console.WriteLine("   "+Convert.ToInt64(v)+" = "+v); }
      else foreach(var p in t.GetProperties(BindingFlags.Public|BindingFlags.Instance))
        Console.WriteLine(string.Format("   {0,-24} : {1,-24} {2}",p.Name,p.PropertyType.Name,p.CanWrite?"(rw)":"(ro)"));
    }
  }
}
